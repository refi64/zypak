...
// An enumeration of all supported D-Bus type codes.
enum class TypeCode {
  kByte = 'y',
  kBool = 'b',
  kInt16 = 'n',
...
// Used as a helper to set the kind member on a trait struct.
...
struct BusTypeTraitsConvertibleBase : BusTypeTraitsBase<KindValue> {
  // The type of the variable in C++ land, i.e. internal to the current application.
  using Internal = InternalType;
  // The type of the variable in libdbus land, i.e. external to the current application.
  using External = ExternalType;
  // Similar to the above, but helps for types like std::string where arguments can be passed as
  // std::string_view.
